...
class BowlingGame(QWidget):
    ...
    def calculate_scores(self):
        for i in range(10):
            ...
            # 10번째 프레임 예외 처리
            if i == 9:
                if frame == ["X", "X", "X"]:  # 10프레임이 XXX일 때
                    self.scores[i] = 30
                elif "X" in frame:  # 스트라이크 포함 (X X 숫자)
                    self.scores[i] = 10 + (10 if frame[1] == "X" else frame[1]) + (10 if frame[2] == "X" else frame[2])
                elif "/" in frame:  # 스페어 포함
                    self.scores[i] = 10 + frame[2]
                else:  # 일반 점수
