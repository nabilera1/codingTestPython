...
ROTATION_SPEED = 5       # 기본 회전 속도 (애니메이션마다 추가되는 각도)
...
def create_wheel_image(size, sectors):
    ...
    font_size = 30  # 숫자 글자 크기 (원하는 크기로 조정 가능)
    try:
...
class SpinningWheelApp:
    def __init__(self, root):
        ...
        self.canvas = tk.Canvas(root, width=IMAGE_SIZE, height=IMAGE_SIZE + 50)
        self.canvas.pack()
        ...
        # --- 캔버스에 버튼 배치 ---
        # 버튼 생성 (부모는 root여도 무방하지만, 캔버스에 embed하기 위해 create_window를 사용)
        self.stop_button = tk.Button(root, text="멈춤", command=self.stop_spinning, font=("Arial", 16))
        self.restart_button = tk.Button(root, text="재시작", command=self.restart_spinning, font=("Arial", 16))
        self.random_speed_button = tk.Button(root, text="랜덤속도", command=self.set_random_speed, font=("Arial", 16))

        # 캔버스 내에 버튼들을 넣습니다.
        # 여기서는 캔버스 하단에 좌측, 중앙, 우측에 각각 배치합니다.
        self.canvas.create_window(80, IMAGE_SIZE + 30, window=self.stop_button)
        self.canvas.create_window(IMAGE_SIZE // 2, IMAGE_SIZE + 30, window=self.restart_button)
        self.canvas.create_window(IMAGE_SIZE - 80, IMAGE_SIZE + 30, window=self.random_speed_button)
    ...
    def update_animation(self):
        """돌림판 회전을 업데이트하는 함수"""
        if self.spinning:
            # 글로벌 회전 속도 사용 (랜덤 속도 버튼에 의해 변경될 수 있음)
    ...
    def restart_spinning(self):
        """재시작 버튼 클릭 시 호출되어 회전을 다시 시작"""
        if not self.spinning:
            self.spinning = True
            self.update_animation()

    def set_random_speed(self):
        """랜덤속도 버튼 클릭 시 호출되어 회전 속도를 무작위로 변경"""
        global ROTATION_SPEED
        ROTATION_SPEED = random.randint(1, 20)  # 1부터 20 사이의 정수 값으로 변경
        print("새로운 랜덤 회전 속도:", ROTATION_SPEED)
