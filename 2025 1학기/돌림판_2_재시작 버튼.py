...
class SpinningWheelApp:
    def __init__(self, root):
        ...
        self.canvas.pack()
        # 버튼들을 담을 프레임 생성, self.canvas
        button_frame = tk.Frame(root)
        button_frame.pack(pady=10)

        # 멈춤 버튼 추가 (프레임 내부에 배치)
        self.stop_button = tk.Button(button_frame, text="멈춤 frame", command=self.stop_spinning, font=("Arial", 16))
        self.stop_button.pack(side=tk.LEFT, padx=5)

        # 재시작 버튼 추가 (프레임 내부에 배치)
        self.restart_button = tk.Button(button_frame, text="재시작 frame", command=self.restart_spinning, font=("Arial", 16))
